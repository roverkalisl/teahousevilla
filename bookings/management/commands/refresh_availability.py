...
class Command(BaseCommand):
    help = "Refresh OTA status records and capture the latest availability sync metadata."
    ...
    def handle(self, *args, **options):
        ...
        if not property_obj:
            self.stdout.write(self.style.WARNING("No Property record exists yet; nothing to refresh."))
            return

        result = refresh_availability_from_ota(property_obj)
        self.stdout.write(self.style.SUCCESS(f"Availability refresh complete for {property_obj.name}: {len(result)} sources processed."))
